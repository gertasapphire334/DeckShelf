...
if [ "$LAUNCHERS" -eq 1 ]; then
  ...
  for f in deckshelf.png deckshelf.ico icon.svg; do
    src=""
  ...
  cat > "$DEST/open-deckshelf.sh" <<OPENER
#!/usr/bin/env bash
...
for b in google-chrome-stable google-chrome chromium chromium-browser brave-browser microsoft-edge vivaldi-stable; do
  if command -v "\$b" >/dev/null 2>&1; then exec "\$b" --app="\$page" --class="DeckShelf"; fi
done
...
OPENER
  chmod +x "$DEST/open-deckshelf.sh"
...
Exec=$DEST/open-deckshelf.sh
Icon=$DEST/deckshelf.png
Terminal=false
Categories=Game;Utility;
StartupWMClass=DeckShelf
DESKTOP
...
BAT
  ...
  say "For a native window, use the DeckShelf executable for your platform — it needs only library.json."
fi
