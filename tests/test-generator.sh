...
[ -f "$OUT/open-deck-shelf.sh" ] || fail "Linux opener is missing"
[ -f "$OUT/icon.svg" ] || fail "SVG icon is missing"
[ -f "$OUT/deck-shelf.png" ] || fail "PNG icon is missing"
[ -f "$OUT/deck-shelf.ico" ] || fail "Windows icon is missing"
bash -n "$OUT/open-deck-shelf.sh"
